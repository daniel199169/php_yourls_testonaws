...
function yourls_redirect_shorturl($url, $keyword) {
    ...
    $table_url = $_SESSION['CUS_YOURLS_DB_TABLE_URL'];

    // echo $table_url;
    // die();

    $conn_ip = mysqli_connect($_SESSION['CUS_YOURLS_DB_HOST'], $_SESSION['CUS_YOURLS_DB_USER'], $_SESSION['CUS_YOURLS_DB_PASS'], $_SESSION['CUS_YOURLS_DB_NAME']);
    if ($conn_ip->connect_error) {
            die("Connection failed: " . $conn_ip->connect_error);
    }else{
            $query_ip = "SELECT * FROM $table_url WHERE keyword= '".$_SESSION['CUS_REQUEST']."'";
        $result_ip = mysqli_query($conn_ip, $query_ip);
        ...
        $location_custom = $details->country." ".$details->region." ".$details->city;
        $latitude = explode(',',$details->loc)[0];
        $longitude = explode(',',$details->loc)[1];
        
        
        mysqli_close($conn_ip);
    ...
    $user_yourls = "yourls";
            ...
            if($conn_yourls){
                 ...
                 $query_location = "UPDATE yourls_log SET latitude = '$latitude' ,longitude = '$longitude' ,location_city = '$location_custom' WHERE shorturl = '".$_SESSION['CUS_REQUEST']."'";
            
                 mysqli_query($conn_yourls, $query_location);
                
                // Past this point this is a request the loader could not understand
                mysqli_close($conn_yourls);
            }else{
    ...
    $user_fm = "ubuntu";
            ...
            if($conn_fm){
                 ...
                 // $temp_url = $_SESSION['CUS_YOURLS_SITE']."/".$keyword;

                 $query_fm = "UPDATE fmessage_bitacora SET bitacora_visto = 1 WHERE bitacora_url_hijo = '".$keyword."'";
            
                 mysqli_query($conn_fm, $query_fm);
                
                // Past this point this is a request the loader could not understand
                mysqli_close($conn_fm);
            }
...
function yourls_log_redirect( $keyword ) {
    ...
    return $ydb->fetchAffected("INSERT INTO $table (click_time, shorturl, referrer, user_agent, ip_address, country_code, latitude, longitude, location_city) VALUES (:now, :keyword, :referrer, :ua, :ip, :location, :latituded, :longituded, :city)", $binds );
